...
这个实验的主任务不是重新训练模型，而是使用 exp2 已经训练好的 FP32 AlexNet，通过 `torch2trt` 构建 TensorRT INT8 engine，然后比较 PyTorch FP32 与 TensorRT INT8 的单图预测结果、平均推理延迟以及完整 CIFAR-10 测试集准确率。CIFAR-10 每张图片是 `32x32` 的 RGB 图片，共有 10 个类别：飞机、汽车、鸟、猫、鹿、狗、青蛙、马、船、卡车。
...
- **Weights 的量化**：代码中没有单独的 `quantize_weights()`。设置 `int8_mode=True` 后，TensorRT builder 在构建 engine 时读取模型中已经训练好的 FP32 weights，为支持 INT8 的层计算权重 scale，并在 engine 中保存/使用量化后的权重。Weights 的数值已经固定，因此确定其范围通常不需要校准图片。
...
6. 再根据 $T$ 推导 FP32 与 INT8 之间的 scale。

以常见的对称有符号 INT8 为例，可以近似写成：

$$scale=\frac{T}{127}, \qquad q=\operatorname{clip}\left(\operatorname{round}\left(\frac{x}{scale}\right),-128,127\right)$$

其中 $x$ 是原始浮点数，$q$ 是 INT8 数值。正式推理时，weights 和 activation values 会按 TensorRT 为各张量确定的动态范围/scale 使用 INT8 表示。
...
可将这一过程概括为：**Weights 的范围可以直接从固定权重得到；activation values 会随输入变化，需要校准集统计分布；熵校准先按 KLD 原则选择截断阈值 $T$，再由 $T$ 计算 INT8 scale。**
...
- 运行前需要准备：
  ...
  - `exp3/weights/alexnet/weights.130.83.050.pt` 权重文件。
- 一键运行后会生成：
  ```text
  exp3/
  ├── test_result.jpg
...
- terminal 和 `run_exp3_out.txt` 会包含：
  ...
  - `Acc for fp32`、`Acc for trt int8`：完整 CIFAR-10 测试集 accuracy；
  - `Total time`：两种模型分别遍历完整测试集的端到端总耗时。
...
- 结果分析时可以重点观察：
  - INT8 prediction 是否与 FP32 相同；
  - confidence 变化是否较小；
  - `INT8 speedup = FP32 average latency / INT8 average latency`；
  - INT8 完整测试集 accuracy 相比 FP32 下降多少；
  - 如果单图类别发生变化或 accuracy 明显下降，需要检查校准图片是否有代表性、校准集和正式输入预处理是否一致，以及部分算子是否发生精度回退。
