...
##################################################################
# 这个脚本用于从 exp1 目录启动训练，并把终端输出保存到 run_exp1_out.txt  #
##################################################################
...
OUT_FILE="$SCRIPT_DIR/run_exp1_out.txt"
# OUT_FILE 是输出日志文件路径。
# 这里表示把日志保存到 exp1/run_exp1_out.txt。
...
#   这样无论是正常打印，还是报错信息，都会一起保存到 run_exp1_out.txt。

# | tee "$OUT_FILE"：
# - | 是管道，把左边命令的输出交给右边命令。
# - tee 会做两件事：
#   - 把输出继续显示在终端上
#   - 同时把输出写入 run_exp1_out.txt
