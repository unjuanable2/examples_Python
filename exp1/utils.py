...
def progress_bar(current, total, msg=None):
    ...
    # 前面已经把整行都写出去了，例如：
    # [========>........]  Step: 20ms | Tot: 3s | Loss: 1.234 | Acc: 56.789%
    # 并且又补了一些空格，把这一行填到接近终端最右边。
    #
    # 所以此时光标大概在“当前这一行”的最右边，不是在下一行。
    # 终端里 write(...) 不会自动换行，除非写入 '\n'。
    #
    # 如果现在直接打印 " 3/391 "：
    # 它会被写在当前行最右边，而不是写在进度条中间。
    #
    # 所以这里先连续输出很多个 '\b'。
    # '\b' 是退格字符，只会让光标在“当前这一行”往左移动，
    # 不会让光标跑到上一行/下一行。
    #
    # 为什么是 term_width - int(TOTAL_BAR_LENGTH / 2) + 2？
    # 可以粗略这样理解：
    # - term_width：终端一整行的宽度，比如 120 列；
    # - 前面补空格后，光标大概在第 120 列，也就是最右边；
    # - TOTAL_BAR_LENGTH 是进度条长度，比如 65；
    # - int(TOTAL_BAR_LENGTH / 2) 是进度条一半，大约 32；
    # - 从最右边往左退 120 - 32 + 2 = 88 次，
    #   光标就会回到靠近进度条中间的位置。
    #
    # 这个公式不是严格数学上的“屏幕正中间”，
    # 而是根据当前进度条长度和终端宽度，估算一个适合写 "3/391" 的位置。
    for i in range(term_width - int(TOTAL_BAR_LENGTH / 2) + 2):
        sys.stdout.write('\b')
        # '\b' 是退格字符，每循环一次，就让光标往左退一格。

    sys.stdout.write(' %d/%d ' % (current + 1, total))
    # 在当前行的进度条中间附近写当前进度数字。
    # current 从 0 开始，所以显示时用 current + 1
    # e.g. 3/391 表示总共 391 个 batch，current = 2, 目前处理到第 3 个
    # 
    # 注意：写完这句后，光标仍然在同一行，只是移动到了 "3/391" 后面。
    # 它不会自动去下一行。
    # 后面的 '\r' 或 '\n' 才负责移动光标：
    # - '\r'：回到当前行开头，下一次进度条覆盖这一行；
    # - '\n'：换到下一行，表示这一条进度条结束。
    ...
    else: # 如果已经是最后一个 batch，就输出换行，结束这一条进度条。
        sys.stdout.write('\n')
