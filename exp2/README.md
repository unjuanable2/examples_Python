...
- `results_analysis/run_exp2_out.txt`：完整训练日志，包含每个 epoch 的 loss 和训练/测试准确率。
- `results_analysis/exp2_epoch_metrics.csv`：从日志自动提取的逐 epoch 指标表，字段与实验 1 的 `exp1_epoch_metrics.csv` 一致。
- `results_analysis/exp2_accuracy_curve.png` / `.svg`：训练和测试准确率曲线。
- `results_analysis/exp2_loss_curve.png` / `.svg`：训练和测试 loss 曲线。
