...
## 结果记录和分析
完成实验后，在 `exp4` 目录运行以下命令即可重新计算本节指标：

```bash
python results_analysis/analysis_exp4.py
```

脚本读取 `detect_original.log`、`prune50.log` 和 `detect_pruned50.log`，把分析结果同时打印到 terminal，并覆盖保存到 `results_analysis/analysis_exp4_log.txt`。脚本路径以自身所在目录为基准，因此也可以进入 `results_analysis` 后运行 `python analysis_exp4.py`。

本次结果来自 NVIDIA GeForce RTX 3060 Laptop GPU（显存 5906 MB）、PyTorch 2.1.2+cu118、输入尺寸 416×416、`conf_thres=0.3`、`nms_thres=0.5`。原模型和剪枝模型使用同一段 `c_test.mp4`，两次都完整处理了 1303 帧，因此视频速度可以直接比较。

| 指标 | 剪枝前/原模型 | 50% 剪枝模型 | 结果解释 |
|---|---:|---:|---|
| 可剪枝通道数 | 25920 | 15180 | 实际减少 10740 个，即 41.44%；shortcut 层共享 mask，因此最终保留数不会恰好等于 50% |
| 参数量 | 61,949,149 | 15,927,814 | 减少 74.29%，剪枝后约为原来的 25.71%，参数压缩约 3.89 倍 |
| COCO 5k mAP@0.5 | 0.481 | 0.510488 | 左侧是代码中写死的老师稀疏模型标注值，右侧才是本次 compact model 实测值，不能把二者当作同条件实测对比 |
| 随机输入前向时间 | 0.0136 s | 0.0109 s | 减少 19.85%，约加速 1.25 倍；左侧实际是保留原结构的剪枝模型，不是 `yolov3-full-mAP53.3.weights` 的实测时间 |
| 平均逐帧 FPS | 73.888 | 87.137 | 提高 17.93%，约为原模型的 1.18 倍 |
| 视频阶段总耗时 | 41.470 s | 25.726 s | 减少 37.96%，剪枝模型完成整个视频阶段约快 1.61 倍 |
| 输出视频大小 | 约 48 MiB | 约 56 MiB | 视频大小受画框内容和编码压缩率影响，不能用它衡量模型是否变小 |

`prune50.log` 显示剪枝阈值为 `0.0000000738`，说明稀疏训练已经把大量 BN gamma 压到非常接近 0。脚本随后完成了三个关键步骤：先用 mask 将低 gamma 通道置零，在 COCO 5k 上验证；再建立物理通道数减少的 compact model 并复制保留参数；最后让 compact model 再次跑完 5000 张图片（313 个 batch）。最终输出 `P=0.0635`、`R=0.71`、`mAP=0.510488`、`F1=0.114`，并成功保存 `cfg/prune_0.5_yolov3.cfg` 和 `weights/prune_0.5_sparse-yolov3-full-mAP48.1.weights`。这表明剪枝、参数复制和最终验证均已正常完成。

Precision 较低而 Recall 较高，主要与当前评估使用较低的置信度阈值有关：保留更多候选框会减少漏检，同时也会增加误检。mAP 会综合不同置信度排序计算 AP，因此不能只根据这里显示的单点 Precision 判断模型失效。从 mAP 和视频能够完整输出看，没有出现通道对应错误或模型结构损坏的迹象。

速度方面，逐帧 FPS 是每帧前向传播、NMS 和相关处理的近似速度，会随画面中的目标数量波动；第一帧还包含 CUDA 初始化开销。这里对日志中的全部 1303 帧取算术平均，原模型为 73.888 FPS，剪枝模型为 87.137 FPS。两个阶段的 `Done` 总耗时还包含模型加载、图片预处理、画框和视频编码，所以总耗时加速倍数与平均 FPS 加速倍数不同，两种数字都正常，但报告时不能混为同一种计时口径。

当前结果目录内容完整：

- `results_analysis/output_original/c_test.mp4`：原模型检测视频，约 48 MiB；
- `results_analysis/output_pruned50/c_test.mp4`：50% 剪枝模型检测视频，约 56 MiB；
- `results_analysis/comparison_prune50.mp4`：左右对比视频，约 21 MiB；
- `results_analysis/detect_original.log`、`prune50.log`、`detect_pruned50.log`：三阶段完整日志。

需要特别注意，当前代码没有在相同环境下重新评估“剪枝前稀疏模型”的 COCO 5k mAP，而是在结果表中直接填入 `0.481`。因此本实验可以可靠得出“参数量显著下降且视频推理加快”，但不能可靠得出“剪枝后 mAP 从 0.481 提升至 0.510488”。若要严格比较精度，应先用同一个 `test.py`、同一份 COCO 5k 和相同参数评估老师提供的稀疏模型，再与 compact model 的 `0.510488` 比较。
...
综合来看，本次 50% 阈值结构化通道剪枝结果正常：生成的 compact model 参数量减少 74.29%，同一视频上的平均逐帧 FPS 提升 17.93%，并且 COCO 5k 验证和三个视频文件都完整生成。仅凭当前日志还不能量化剪枝带来的精度变化，也不能从日志判断具体哪些小目标发生漏检；这两项分别需要补测剪枝前稀疏模型的 COCO 5k 指标，以及逐帧观看左右对比视频后记录典型画面。
