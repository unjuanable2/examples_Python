"""YOLOv3 稀疏训练和结构化通道剪枝的核心辅助函数。

本实验主要使用：parse_module_defs2 找到可剪枝层及 shortcut 对应关系，
gather_bn_weights 收集 gamma，obtain_bn_mask 生成保留/删除掩码，
prune_model_keep_size2 传播被删通道的偏移，init_weights_from_loose_model 把保留
参数复制到真正缩短通道的紧凑模型，write_cfg/save_weights 最终保存结果。
"""
...
#check each conv module for pruning
def parse_module_defs2(module_defs):

    CBL_idx = []
    Conv_idx = []
    shortcut_idx = dict()
    shortcut_all = set() #only leave one copy of the element
    ignore_idx = set()

    for i, module_def in enumerate(module_defs): # bianli the list 
        if module_def['type'] == 'convolutional':
            if module_def['batch_normalize'] == '1':
                CBL_idx.append(i)
            else:
                Conv_idx.append(i)
        elif module_def['type'] == 'upsample': #upsample index is i ,previous conv idex is i-1
            ignore_idx.add(i-1)
        elif module_def['type'] == 'shortcut':
            identity_idx = (i + int(module_def['from']))
            if module_defs[identity_idx]['type'] == 'convolutional':
                shortcut_idx[i-1] = identity_idx #identi = 2, shortcut_idx[4] = 2
                shortcut_all.add(identity_idx)
            elif module_defs[identity_idx]['type'] == 'shortcut':
                shortcut_idx[i-1] = identity_idx-1 #shortcut_idx[i-1] = i-4
