"""图片、视频流和带 YOLO 标签数据集的读取与预处理。

``detect.py`` 使用 LoadImages/LoadStreams：读取帧、letterbox 到 416、BGR 转 RGB、
像素归一化到 0~1。``test.py`` 使用 LoadImagesAndLabels：除图片外还把路径中的
images 替换成 labels，读取同名 txt ground truth，供 COCO mAP 计算。
"""
...
# 只有这里列出的扩展名会被识别，因此 iPhone MOV 和转码后的 MP4 都能直接使用。
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=True, image_weights=False,
        ...
        if self.rect:
            ...
            self.batch_shapes = np.ceil(np.array(shapes) * img_size / 32.).astype(np.int_) * 32
...
def letterbox(img, new_shape=416, color=(128, 128, 128), mode='auto', interp=cv2.INTER_AREA):
    ...
    # 字符串内容应使用 == 比较；``is`` 比较对象身份，在不同 Python 版本中结果不可靠。
...
def convert_images2bmp():
    ...
    for path in ['/opt/DATASET/coco/images/val2014/', '/opt/DATASET/coco/images/train2014/']:
    ...
    for label_path in ['/opt/DATASET/coco/trainvalno5k.txt', '/opt/DATASET/coco/5k.txt']:
