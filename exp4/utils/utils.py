...
模型构建、检测、验证和剪枝都会 import 本模块。与作业最相关的函数包括：
load_darknet_weights/save_weights 读写 Darknet 权重，non_max_suppression 做 NMS，
scale_coords 把输入尺寸坐标还原到视频尺寸，plot_one_box 画检测框，ap_per_class
根据 COCO 5k 的预测/真值统计每个类别 AP。
"""
...
def coco_class_count(path='/opt/DATASET/coco/labels/train2014/'):
...
def coco_only_people(path='/opt/DATASET/coco/labels/val2014/'):
...
def coco_single_class_labels(path='/opt/DATASET/coco/labels/train2014/', label_class=43):
...
def kmeans_targets(path='/opt/DATASET/coco/trainvalno5k.txt', n=9, img_size=416):  # from utils.utils import *; kmeans_targets()
