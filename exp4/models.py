"""把 Darknet ``.cfg`` 转为 PyTorch YOLOv3，并负责前向传播和权重读写。

``detect.py``、``test.py`` 和 ``shortcut_prune.py`` 都会创建这里的 ``Darknet``。
最关键的调用链是 ``parse_model_cfg -> create_modules -> Darknet.forward``：先把 cfg
的每个文本块变成卷积、shortcut、route、upsample 或 YOLOLayer，再按网络顺序计算。
剪枝后 cfg 的 filters 数量变小，因此同一份代码可以构建真正更窄、更快的模型。
"""
...
def create_modules(module_defs, img_size, arc):
    """把 cfg 解析得到的字典列表转换为 ``nn.ModuleList``。

    module_defs 描述“层是什么、参数是多少”；module_list 保存真正参与 PyTorch
    计算的层对象。两者下标保持对应，这是剪枝代码能够按层复制权重的基础。
    """
